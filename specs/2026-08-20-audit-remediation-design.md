...
9. Boundary gaps: the `--terms` list has no canonical location and no
   `.gitignore` net; a city name reached eight tracked files through a CSS
...
| D2 | Is the roadshow pitch deck to be scored (corpus D18) in this cycle? | generate the sheet; scoring is hers, not blocking |
