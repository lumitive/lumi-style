...
python3 scripts/check_design.py <file>   # design metrics (D1-D20) on a deliverable
...
exits non-zero on the eleven findings that are decidable rather than aesthetic
(collision, content spill, page height, hidden content, a wrapped footer, a
viewBox that does not parse, a drawing clipped by its own viewBox, a seigaiha
band under evidence, an overspent title reserve, a role split, a lost datum)
and is the pre-delivery step in `SKILL.md`.
...
`UNMEASURABLE` rather than passed. Five of its metrics **gate**, and none is a
...
(no repository path in a footer) is D12's mirror, **D19** (every `<use>`
resolves, every block pattern renders) asks whether the document can render
its own markup, and **D20** (expressive vocabulary only under a declared
`data-register="expressive"`, one illustration per page) asks whether the
document says what it is. All five are
...
- `references/eval-rubric.md` — M1–M12 / D1–D20 / H1–H6 scoring rubric and the review protocol (the iteration engine)
