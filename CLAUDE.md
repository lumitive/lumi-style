...
exits non-zero on the twelve findings that are decidable rather than aesthetic
(collision, a starved column, content spill, page height, hidden content, a
wrapped footer, a viewBox that does not parse, a drawing clipped by its own
viewBox, a seigaiha band under evidence, an overspent title reserve, a role
split, a lost datum)
