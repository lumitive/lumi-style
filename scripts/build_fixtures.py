...
def region_bindings() -> str:
    """Bind `.rg-<id>` and the four state classes to the shipped `--rg-*` values.

    `globe_svg.py` emits `class="rg rg-europe is-live"` and `region-palette.css`
    ships `--rg-europe`, `--rg-europe-stroke` and `--rg-europe-wash`. NOTHING IN
    tokens/ JOINS THE TWO, so a document that does not write this block draws
    every region in the UA default — black. design-rules.md section 1 puts that
    on the document ("every shape carries a class and the host document paints
    it from tokens"), so this is the fixture doing a deliverable's job rather
    than reimplementing a shipped rule.

    GENERATED from the palette file, not typed, so a region added to the
    registry cannot leave a black shape here. Whether tokens/ should ship this
    block itself, so no deliverable has to write eleven regions by hand, is a
    design decision and is recorded in the changelog rather than taken here.
    """
    palette = (ROOT / "tokens/region-palette.css").read_text(encoding="utf-8")
    ids = sorted({m for m in re.findall(r"--rg-([a-z-]+?):", palette)
                  if not m.endswith(("-stroke", "-wash"))})
    out = ["\n/* region fills, generated by scripts/build_fixtures.py */",
           ".rg { stroke-width: 1; }"]
    for rid in ids:
        out.append(f".rg-{rid} {{ fill: var(--rg-{rid}); "
                   f"stroke: var(--rg-{rid}-stroke); }}")
        out.append(f".rg-{rid}.is-zero {{ fill: var(--rg-{rid}-wash); }}")
    # State overrides the hue for the two states that are not about identity.
    out.append(".rg.is-out { fill: var(--brass); }")
    out.append(".rg.is-partial { stroke: var(--amber); stroke-width: 2; }")
    return "\n".join(out) + "\n"
