...
is a classic script with no fetch, no module graph and no network.
    ...
    python3 scripts/embed_globe.py > block.html          # runtime + geometry
    python3 scripts/embed_globe.py --states '{"europe":"live"}'
    python3 scripts/embed_globe.py --no-autorotate
    python3 scripts/embed_globe.py --check               # can it build, and is it sane
...
Pair it with scripts/globe_svg.py, which emits the markup this mutates. Without
...
def build(states=None, autorotate=True, form="field"):
    ...
    host = {"regions": {k: {"state": v} for k, v in (states or {}).items()}}

    return f"""<script>
/* LUMI globe runtime — GENERATED by scripts/embed_globe.py. Do not edit here.
 *
 ...
 * the markup scripts/globe_svg.py emitted and never creates any, so with
  ...
  function boot() {{
    ...
    for (var i = 0; i < figures.length; i += 1) {{
      createGlobe(figures[i], {{
        ...
        registry: REGISTRY,
        hostData: HOSTDATA,
        form: figures[i].dataset.globe || {json.dumps(form)},
...
def check():
    errors = []
    try:
        block = build(states={"europe": "live"})
...
def main(argv):
    ...
    ap.add_argument("--states", metavar="JSON", default=None,
                    help='region states, e.g. \'{"europe":"live"}\'')
    ap.add_argument("--form", choices=("field", "regions"), default="field",
                    help="the form a figure starts in; an element may override "
                         "it with data-globe")
    ...
    print(build(states=states, autorotate=not args.no_autorotate, form=args.form))
    return 0
