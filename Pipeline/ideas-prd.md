...
|---|---|---|---|
| 1 | Implement M1, M2 and M6 | Three unmeasured metrics stand behind a *fact* red line, which the rules say outranks every style rule | Medium |
| 2 | Make Chinese a supported output path | Every English rule has machine backing; no Chinese rule does, for a team that writes Chinese | Large |
| 3 | Assert the rendered gates on the fixtures | Nine gates and thirteen design metrics have no failing case in CI | Medium |
| 4 | Store the reader scores | The iteration engine keeps no data; the human half is anecdote | Small |
| 5 | Run the two unused viewports by default | §7 requires two checks that never run unless asked for by hand | Small |
| 6 | Fill or retire the `files` capability tier | A tier defined, restated in three places, and used by nothing | Small |
| 7 | Make the conformance board mean something | One agent of twelve, n=1, and an unscored run on disk | Medium |

Items 1 and 2 are the structural gaps. Items 3 to 7 are hygiene, and 3 is the one
that protects everything else.

---

## 1 · Implement M1, M2 and M6
...
## 2 · Make Chinese a supported output path
...
## 3 · Assert the rendered gates on the fixtures
...
## 5 · Run the two unused viewports by default
...
## 6 · Fill or retire the `files` capability tier
...
## 7 · Make the conformance board mean something
